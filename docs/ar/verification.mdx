...
title: Verification
description: >-
  التحقق من تكوين نفق GRE وتكوين BGP على BIG-IP، واستكشاف المشكلات الشائعة
  وإصلاحها.
sidebar:
  order: 6
i18n:
...
قم بالتشغيل على كل وحدة للأنفاق الخاصة بها:
...
:::caution[قائمة فحص استكشاف الأخطاء وإصلاحها]
إذا لم يتم إنشاء اتصال BGP:

1. تحقق من أن نفق GRE يعمل: يجب أن يُظهر `show net tunnels tunnel`
   الحالة `up`.
1. تأكد من أن `allow-service` يتضمن `tcp:179` على عنوان Self IP الداخلي.
1. قم بعمل ping لعنوان IP الداخلي البعيد عبر النفق.
1. تحقق من تطابق كلمة مرور BGP على كلا الجانبين (سبب شائع لأخطاء
   `NOTIFICATION`).
1. تحقق من تطبيق prefix-lists وroute-maps بشكل صحيح.
1. تحقق من `show ip bgp neighbors <IP>` لمعرفة آخر سبب خطأ/إعادة
   تعيين.
1. على كل وحدة HA، تحقق من أن أنفاق الوحدة الخاصة بها تعمل وأن
   جيران BGP الخاصين بها مُكوَّنون (وليس تلك الخاصة بالوحدة الأخرى).
:::

### وحدة تحكم XC

انتقل إلى **DDoS and Transit Services >
[DDoS Protection > Visibility](https://docs.cloud.f5.com/docs-v2/ddos-and-transit-services/how-tos/network-firewall/l3l4-ddos-mitigation)** لعرض:

- حركة المرور حسب الشبكة والمنطقة والتطبيق.
- حركة المرور المحظورة مقابل المسموح بها.

...
de los objetos y las IPs según sea necesario.
...
inicial de enrutamiento con el Cloud, consulte [K000147949](https://my.f5.com/manage/s/article/K000147949).
...
depuración del Cloud. Cree **dos túneles por unidad** (uno hacia
...
La encapsulación GRE añade sobrecarga (24 bytes para IPv4 exterior, 44 bytes
para IPv6 exterior). Sin un MTU explícito, los paquetes cercanos a 1500 bytes
se fragmentarán o se descartarán. Configure el MTU del túnel para tener en cuenta la
...
:::caution[Path MTU]
Los valores anteriores asumen un Path MTU de 1500 bytes en la red exterior.
...
**no** estén bloqueados en ningún dispositivo intermedio para que el Path MTU
...
a las IPs de origen esperadas de los centros de depuración del Cloud:
...
se aplica a las ACLs IPv6 para los túneles GRE IPv6. Tanto las IPs
externas de BIG-IP-A (`xBIGIP_A_OUTER_V4x`) como de BIG-IP-B
(`xBIGIP_B_OUTER_V4x`) deben estar permitidas como destinos GRE.
:::

### Self IPs internas (peering BGP)

Asigne direcciones IP internas (dentro del túnel GRE) que formarán la
**sesión BGP** con el Cloud. El `allow-service` debe
incluir `tcp:179` (BGP) para que la sesión de peering se establezca. Agregar
...
Utilice [imish](https://techdocs.f5.com/kb/en-us/products/big-ip_ltm/manuals/product/big-ip-tmos-routing-administration-14-0-0/11.html) para configurar BGP para el Route Domain 0.
...
- `xCUSTOMER_ASNx` — su ASN público registrado en ARIN (o el
  ...
  porque el Cloud anunciará sus prefijos globalmente.
  Un ASN privado (64512-65534 / 4200000000-4294967294) solo es
  aceptable si el SOC admite explícitamente peering con ASN privado para
  su despliegue.
- `xF5_XC_ASNx` — ASN del Cloud (proporcionado por el SOC).
- `xBGP_PASSWORDx` — la contraseña MD5 de BGP acordada (o que coincida con el
...
**Configuraciones clave de BGP explicadas:**

- **`timers 10 30`** — Keepalive cada 10 s, tiempo de espera (hold time) 30 s. El
...
- **`ttl-security hops 1`** — Habilita GTSM
  ...
  sobre el túnel GRE, esto previene el spoofing BGP remoto al
  requerir TTL = 255 en los paquetes BGP entrantes.
- **`maximum-prefix 10 warning-only`** — Protección de defensa en profundidad.
...
- **`redistribute kernel`** — Inyecta las rutas estáticas `null0` en
  BGP a través del route-map. Una alternativa es usar sentencias `network`
  explícitas (por ejemplo, `network xPROTECTED_PREFIX_V4x`), que son más precisas
...
Confirme el soporte de BFD con su versión de BIG-IP y el SOC antes
de habilitarlo.
:::

Las rutas estáticas `null0` con una distancia administrativa más alta (201)
aseguran que los prefijos existan en la **tabla de enrutamiento del kernel**, para que puedan
ser redistribuidos al Cloud a través de BGP sin afectar

---
title: Vue d'ensemble
description: >-
  Introduction à la configuration des tunnels GRE et du peering BGP depuis une
  paire HA BIG-IP vers les centres de nettoyage Cloud DDoS.
sidebar:
  order: 1
i18n:
  sourceHash: 8a2fdcdca9db
  translator: machine
---

## Cloud GRE/BGP BIG-IP

- Configurez les **tunnels GRE** et le **peering BGP** depuis une paire HA BIG-IP
  (agissant comme équipement local client, CPE), avec des tunnels
  indépendants par unité.
- Connectez-vous au service de **mitigation DDoS Cloud** dans les centres
  de nettoyage en **mode routé** (L3/L4).

```mermaid
flowchart LR
    INET["Internet<br/>Trafic entrant"]

    subgraph XC["F5 Distributed Cloud<br/>Global Anycast"]
        SCRUB["Nettoyage DDoS<br/>Mitigation L3/L4"]
        DROP["Trafic d'attaque<br/>Rejeté"]
    end

    subgraph DC["Centre de données xDC_NAMEx"]
        BIGIP["BIG-IP CPE<br/>(point de terminaison GRE)"]
        SERVERS["Serveurs protégés contre les DDoS<br/><i>Votre bloc d'IP publiques</i>"]
    end

    INET -->|"tout le trafic vers vos<br/>IP publiques"| SCRUB
    SCRUB -->|"trafic propre"| BIGIP
    SCRUB -.->|"trafic malveillant"| DROP
    BIGIP --> SERVERS
    SERVERS --> BIGIP
    BIGIP -.->|"trafic retour<br/>via liaison montante ISP<br/>(chemin asymétrique)"| INET

    style SERVERS fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
```

## Prérequis

- Service de **mitigation DDoS routée L3/L4** Cloud
  (Always On ou Always Available) activé pour votre tenant.
- BIG-IP avec :
    - LTM (ou modules réseau équivalents).
    - **Routage dynamique (BGP)** licencié et activé.
- Mode routé : au moins un préfixe **publiquement annoncé en /24 (ou plus court)**
  pour la protection (le minimum IPv6 est **/48**).
    - Les préfixes protégés **doivent être publiquement routables** (non-RFC 1918).
     Les points de terminaison externes GRE doivent également être publiquement routables lorsque les tunnels
     traversent l'Internet public ; les déploiements utilisant une connectivité
     privée (L2, peering privé) peuvent utiliser des adresses de points de terminaison
     RFC 1918.
- Connectivité entre votre centre de données/routeur et le(s)
  centre(s) de nettoyage Cloud.

## Architecture HA

Le BIG-IP est déployé en tant que **paire HA active/standby**, chaque unité
dispose de ses propres tunnels GRE indépendants et sessions BGP vers chaque
centre de nettoyage :

```mermaid
flowchart LR
    subgraph F5XC["F5 Distributed Cloud"]
        C1["Centre de nettoyage xCENTER_1x"]
        C2["Centre de nettoyage xCENTER_2x"]
    end

    subgraph DC["Centre de données xDC_NAMEx"]
        direction TB
        subgraph UNITA["BIG-IP-A (Actif)"]
            A_C1["Tunnel C1-T1<br/>BGP Établi"]
            A_C2["Tunnel C2-T1<br/>BGP Établi"]
        end
        subgraph UNITB["BIG-IP-B (Standby)"]
            B_C1["Tunnel C1-T2<br/>Graceful-Restart Prêt"]
            B_C2["Tunnel C2-T2<br/>Graceful-Restart Prêt"]
        end
        SERVERS["Serveurs protégés contre les DDoS"]
    end

    C1 -- "GRE C1-T1" --> A_C1
    C2 -- "GRE C2-T1" --> A_C2
    C1 -- "GRE C1-T2" --> B_C1
    C2 -- "GRE C2-T2" --> B_C2
    UNITA --> SERVERS
    UNITB --> SERVERS

    style SERVERS fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
```

- **Points de terminaison de tunnel indépendants** : Chaque unité BIG-IP possède sa propre
  IP self externe non flottante (`traffic-group-local-only`) et son
  propre ensemble de tunnels GRE. BIG-IP-A utilise `xBIGIP_A_OUTER_V4x` et
  BIG-IP-B utilise `xBIGIP_B_OUTER_V4x` comme points de terminaison de tunnel. Cela évite
  la dépendance à une IP flottante pour le sourçage des tunnels.
- **Sessions BGP indépendantes** : Chaque unité exécute ses propres sessions BGP
  sur ses propres tunnels. BIG-IP-A s'appaire avec C1-T1 et C2-T1 ;
  BIG-IP-B s'appaire avec C1-T2 et C2-T2. Lors d'un basculement, les sessions
  BGP de l'unité standby sont déjà établies, permettant au
  Cloud de rediriger le trafic immédiatement.
- **Synchronisation de configuration** : Les configurations de tunnels, d'IP self et de routage sont
  synchronisées entre les unités via **config-sync**. Étant donné que la
  configuration BGP `imish` est par unité, chaque unité maintient ses propres
  déclarations de voisins. Vérifiez que la synchronisation inclut tous les objets tmsh.
- **Comportement BGP actif/standby** : L'unité active annonce
  les préfixes protégés avec des attributs BGP normaux. L'unité standby
  peut soit annoncer les mêmes préfixes avec un AS-path prepend plus long
  (la rendant moins préférée), soit supprimer les annonces
  jusqu'au basculement. Coordonnez l'approche avec le SOC.
- **Convergence lors du basculement** : Avec `graceful-restart` activé et
  des tunnels indépendants, la nouvelle unité active dispose déjà de sessions
  BGP établies. La convergence dépend du déplacement de la sélection du meilleur chemin BGP
  vers les annonces de l'unité nouvellement active. Testez avec
  `run sys failover standby`.

:::note
Le modèle HA à tunnels indépendants décrit ci-dessus est l'approche recommandée
pour la redondance des équipements côté client. Validez votre conception
de basculement spécifique avec votre équipe de compte avant de passer en
production, en particulier concernant la stratégie d'AS-path prepend et le
timing de reconvergence BGP.
:::

---
title: Verification
description: BIG-IP에서 GRE 터널 및 BGP 구성을 확인하고 일반적인 문제를 해결합니다.
sidebar:
  order: 6
i18n:
...
1. `show ip bgp neighbors <IP>`를 확인하여 마지막 오류/리셋
   원인을 파악합니다.
1. 각 HA 유닛에서 해당 유닛 자체의 터널이 활성화되어 있고 해당 유닛
   자체의 BGP 네이버가 구성되어 있는지(다른 유닛의 것이 아닌) 확인합니다.
:::

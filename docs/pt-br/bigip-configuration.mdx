...
Todos os comandos abaixo são executados no **tmsh** do BIG-IP. Ajuste os nomes dos objetos e os IPs conforme necessário.

Para a configuração geral de túneis GRE no BIG-IP, consulte
[Configuring a GRE Tunnel Using BIG-IP](https://community.f5.com/kb/technicalarticles/configuring-a-generic-routing-encapsulation-gre-tunnel-using-big-ip/289030). Para a configuração inicial de roteamento com o Cloud, consulte [K000147949](https://my.f5.com/manage/s/article/K000147949).
...
Cada túnel aponta de uma unidade BIG-IP para um endpoint de centro de limpeza do Cloud. Crie **dois túneis por unidade** (um para cada centro de limpeza geograficamente localizado) para um total de **quatro túneis lógicos** no par HA:
...
O encapsulamento GRE adiciona overhead (24 bytes para IPv4 externo, 44 bytes para IPv6 externo). Sem um MTU explícito, pacotes próximos de 1500 bytes serão fragmentados ou descartados. Defina o MTU do túnel para considerar o overhead do encapsulamento:
...
Ajuste caso seu Path MTU upstream seja menor. Certifique-se de que o ICMP
...
apenas aos IPs de origem esperados dos centros de limpeza do Cloud:
...
:::note
Adapte a sintaxe para a plataforma do seu roteador. O mesmo princípio
...
`icmp:any` nos self IPs internos habilita o PMTUD e testes de
alcançabilidade através do túnel:
...
A configuração BGP no `imish` é local para cada unidade BIG-IP. Cada
unidade configura apenas os neighbors de seus próprios túneis:
...
:::caution[Substitua os valores de marcação]
- `xCUSTOMER_ASNx` — seu ASN público registrado no ARIN (ou
  ...
  aceitável se o SOC suportar explicitamente peering com ASN privado
...
- **`timers 10 30`** — Keepalive a cada 10 s, hold time de 30 s. O
  valor padrão (60 / 180) é muito lento para failover de mitigação DDoS.
  Coordene os valores dos timers com o SOC para que ambos os lados coincidam.
- **`ttl-security hops 1`** — Habilita o GTSM
  ...
  através do túnel GRE, isso previne spoofing BGP remoto exigindo
  TTL = 255 nos pacotes BGP de entrada.
- **`maximum-prefix 10 warning-only`** — Proteção em profundidade.
...
- **`redistribute kernel`** — Injeta as rotas estáticas `null0` no
  BGP via route-map. Uma alternativa é usar declarações `network`
  explícitas (ex.: `network xPROTECTED_PREFIX_V4x`), que são mais precisas
  pois apenas o prefixo exato é anunciado independentemente de outras
  rotas do kernel. Ambas as abordagens funcionam; `redistribute kernel` com um
  route-map restrito é mostrado aqui por flexibilidade.

:::tip[BFD (Bidirectional Forwarding Detection)]
Se a versão do seu BIG-IP suportar BFD sobre túneis GRE, habilite-o
...
O BFD fornece convergência mais rápida do que apenas os timers de keepalive do BGP.
Confirme o suporte ao BFD com a versão do seu BIG-IP e com o SOC antes
de habilitar.
:::
...
já existir na tabela de roteamento proveniente de outra fonte com distância

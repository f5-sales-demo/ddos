...
description: >-
  ไดอะแกรมโทโพโลยีเครือข่ายและเวิร์กชีตการกำหนด IP address สำหรับการกำหนดค่า GRE
  tunnel
sidebar:
  order: 3
i18n:
...
การกำหนดค่าสำหรับดาต้าเซ็นเตอร์ **xDC_NAMEx**
ที่เชื่อมต่อกับศูนย์ scrubbing บนคลาวด์

:::note
**ค่าเหล่านี้เป็นตัวอย่างเท่านั้น** ให้แทนที่ด้วยค่าเฉพาะของลูกค้าและ
ค่าที่ SOC จัดเตรียมให้โดยใช้ตารางด้านบน

Protected prefixes **ต้องสามารถ route ได้แบบสาธารณะ** (ไม่ใช่ RFC 1918)
IP ของ GRE outer endpoint ต้องสามารถ route ได้แบบสาธารณะเช่นกันเมื่อ tunnel
ผ่านอินเทอร์เน็ตสาธารณะ การเชื่อมต่อแบบส่วนตัว (L2, private
peering) อาจอนุญาตให้ใช้ endpoint แบบ RFC 1918 ได้ ดู
[K000147949](https://my.f5.com/manage/s/article/K000147949) สำหรับตัวอย่างการใช้ที่อยู่เอกสารอ้างอิงที่ถูกต้อง

เพื่อความซ้ำซ้อน ให้สร้าง **tunnel 2 อันต่อ BIG-IP หนึ่งหน่วย** ไปยังศูนย์ scrubbing
ที่อยู่คนละพื้นที่ทางภูมิศาสตร์ (รวม 4 tunnel สำหรับคู่ HA)
:::
...
IP ภายใน เช่น `10.10.10.0/30` ใช้ที่อยู่ RFC 1918 ซึ่งถูกต้อง
เนื่องจากถูกห่อหุ้ม (encapsulate) อยู่ภายใน GRE tunnel และไม่ปรากฏ
บนอินเทอร์เน็ตสาธารณะ Protected prefixes ต้องสามารถ route ได้แบบ
สาธารณะเสมอ IP ของ outer endpoint ต้องสามารถ route ได้แบบสาธารณะ
เมื่อ tunnel ผ่านอินเทอร์เน็ตสาธารณะ
:::

:::note[ลิงก์ IPv6 ภายใน]
ลิงก์ IPv6 ภายในใช้ prefix /64 ในที่นี้เพื่อให้ตรงกับค่าเริ่มต้นทั่วไป
ของคลาวด์ สำหรับลิงก์แบบ point-to-point แนะนำให้ใช้ /127 ตาม
[RFC 6164](https://datatracker.ietf.org/doc/html/rfc6164) เพื่อหลีกเลี่ยงปัญหา neighbor-discovery exhaustion ให้ใช้ /127

...
受保护前缀**必须是可公网路由的**（非 RFC 1918）。
当隧道经过公共互联网时，GRE 外部端点 IP 也必须是可公网路由的；私有连接（L2、私有对等互联）可以允许使用 RFC 1918 端点。请参阅
[K000147949](https://my.f5.com/manage/s/article/K000147949) 了解使用正确文档地址的示例。

为实现冗余，每个 BIG-IP 设备创建 **2 条隧道**，分别连接到不同地理位置的清洗中心（HA 对共 4 条隧道）。
:::
...
在构建隧道配置时，请使用以下 XC 和 BIG-IP 工作表作为参考。
...
:::note[内部（中转）IP]
内部 IP（如 `10.10.10.0/30`）使用 RFC 1918 地址。这是正确的，因为它们被封装在 GRE 隧道内部，不会出现在公共互联网上。受保护前缀必须始终是可公网路由的；当隧道经过公共互联网时，外部端点 IP 必须是可公网路由的。
:::

:::note[IPv6 内部链路]
此处 IPv6 内部链路使用 /64 前缀，以匹配常见的云默认配置。对于点对点链路，根据 [RFC 6164](https://datatracker.ietf.org/doc/html/rfc6164)，推荐使用 /127 以避免邻居发现耗尽问题。如果 SOC 隧道分配支持，请使用 /127。
:::

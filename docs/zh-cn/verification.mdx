...
1. 通过隧道 ping 远端内部 IP。
1. 检查两端的 BGP 密码是否匹配（这是导致 `NOTIFICATION` 错误的常见原因）。
1. 验证 prefix-list 和 route-map 是否正确应用。
1. 检查 `show ip bgp neighbors <IP>` 获取最后的错误/重置原因。
1. 在每个 HA 单元上，验证该单元自身的隧道是否已启动，以及其自身的 BGP 邻居是否已配置（而非另一个单元的）。
:::

### XC 控制台

导航至 **DDoS and Transit Services >
[DDoS Protection > Visibility](https://docs.cloud.f5.com/docs-v2/ddos-and-transit-services/how-tos/network-firewall/l3l4-ddos-mitigation)** 查看：

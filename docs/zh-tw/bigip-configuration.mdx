---
title: BIG-IP 設定
description: 用於在 BIG-IP HA 配對上設定 GRE 通道、Self IP 和 BGP 的 tmsh 與 imish 命令。
sidebar:
  order: 5
i18n:
...
以下所有命令均在 BIG-IP 上的 **tmsh** 中執行。請根據需要調整物件名稱和 IP。

有關 BIG-IP 上的一般 GRE 通道設定，請參閱
[Configuring a GRE Tunnel Using BIG-IP](https://community.f5.com/kb/technicalarticles/configuring-a-generic-routing-encapsulation-gre-tunnel-using-big-ip/289030)。有關與 Cloud 的初始路由設定，請參閱 [K000147949](https://my.f5.com/manage/s/article/K000147949)。
...
這些是每個 BIG-IP 單元上用作 **GRE 通道端點**的 IP，通常位於外部 VLAN 上。每個單元都有自己的非浮動外部 self IP（`traffic-group-local-only`）：
...
每個通道從一個 BIG-IP 單元指向一個 Cloud 清洗中心端點。為每個單元建立**兩個通道**（分別指向每個地理位置的清洗中心），HA 配對共計**四個邏輯通道**：

**通道 C1-T1 — BIG-IP-A 到 xCENTER_1x：**
...
**通道 C2-T1 — BIG-IP-A 到 xCENTER_2x：**
...
**通道 C1-T2 — BIG-IP-B 到 xCENTER_1x：**
...
**通道 C2-T2 — BIG-IP-B 到 xCENTER_2x：**
...
通道名稱（`xc-ddos-c1t1-v4` 等）可任意命名；請使用您自己的命名慣例。

### 設定通道 MTU

GRE 封裝會增加額外負擔（IPv4 外部標頭增加 24 位元組，IPv6 外部標頭增加 44 位元組）。若未明確設定 MTU，接近 1500 位元組的封包將會分片或被丟棄。請設定通道 MTU 以考慮封裝的額外負擔：
...
:::caution[路徑 MTU]
上述數值假設外部網路的路徑 MTU 為 1500 位元組。若您的上游路徑 MTU 更小，請進行調整。請確保任何中間設備上未**阻擋** ICMP "destination unreachable / fragmentation needed"（類型 3，代碼 4），以使路徑 MTU 探索（PMTUD）能正常運作。
:::

### GRE 反欺騙（上游 ACL）

GRE（IP 協定 47）不提供驗證功能。任何知道外部 IP 配對的人都可以將流量注入通道。請在上游路由器或防火牆上套用 ACL，將入站 GRE 限制為僅允許預期的 Cloud 清洗中心來源 IP：
...
:::note
請根據您的路由器平台調整語法。相同原則適用於 IPv6 GRE 通道的 IPv6 ACL。BIG-IP-A（`xBIGIP_A_OUTER_V4x`）和 BIG-IP-B（`xBIGIP_B_OUTER_V4x`）的外部 IP 都必須被允許作為 GRE 目的地。
:::

### 內部 Self-IP（BGP 對等）

指派內部 IP 位址（在 GRE 通道內部），用於與 Cloud 建立 **BGP 工作階段**。`allow-service` 必須包含 `tcp:179`（BGP），以便對等工作階段能夠建立。在內部 self IP 上新增 `icmp:any` 可啟用通過通道的 PMTUD 和連線測試：
...
:::note[每單元設定]
`imish` BGP 設定是每個 BIG-IP 單元本機的設定。每個單元只為自己的通道設定鄰居：

- **BIG-IP-A** 設定 C1-T1 和 C2-T1 鄰居。
- **BIG-IP-B** 設定 C1-T2 和 C2-T2 鄰居。

`router-id` 必須每個單元唯一（使用各單元自己的外部 self IP）。
:::
...
:::caution[替換預留值]
- `xCUSTOMER_ASNx` — 您在 ARIN（或等效 RIR）註冊的公有 ASN。路由式 DDoS **需要公有 ASN**，因為 Cloud 將在全球通告您的前綴。私有 ASN（64512-65534 / 4200000000-4294967294）僅在 SOC 明確支援您的部署使用私有 ASN 對等時才可接受。
- `xF5_XC_ASNx` — Cloud ASN（由 SOC 提供）。
- `xBGP_PASSWORDx` — 約定的 BGP MD5 密碼（或與 Console「default secret」匹配）。請勿將此密碼重複用於其他服務。
:::
...
- **`timers 10 30`** — 每 10 秒發送 Keepalive，保持時間為 30 秒。預設值（60 / 180）對於 DDoS 緩解故障轉移來說太慢。請與 SOC 協調計時器值以確保兩端匹配。
- **`ttl-security hops 1`** — 啟用 GTSM（[RFC 5082](https://datatracker.ietf.org/doc/html/rfc5082)）。由於 BGP 工作階段是透過 GRE 通道的單跳連線，此設定要求傳入 BGP 封包的 TTL = 255，藉此防止遠端 BGP 欺騙。
- **`maximum-prefix 10 warning-only`** — 縱深防禦保障措施。即使入站前綴列表拒絕所有路由，當對等方意外發送前綴時仍會產生警告。
- **`redistribute kernel`** — 透過 route-map 將 `null0` 靜態路由注入 BGP。替代方案是使用明確的 `network` 陳述式（例如 `network xPROTECTED_PREFIX_V4x`），這更精確，因為無論其他核心路由如何，都只會通告確切的前綴。兩種方法都可行；此處展示的是使用嚴格 route-map 的 `redistribute kernel` 以提供彈性。
...
BFD 比單獨的 BGP keepalive 計時器提供更快的收斂速度。啟用前請與您的 BIG-IP 版本和 SOC 確認 BFD 支援情況。
:::

具有較高管理距離（201）的 `null0` 靜態路由確保前綴存在於**核心路由表**中，因此可以透過 BGP 重新分配到 Cloud，而不影響非攻擊條件下的正常路由。如果受保護的前綴已從另一個來源以較低管理距離存在於路由表中，`null0` 路由將不會生效，重新分配可能會失敗 — 設定完成後請使用 `show ip route` 進行驗證。

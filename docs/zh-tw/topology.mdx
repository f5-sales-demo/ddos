---
title: 拓撲與工作表
description: GRE 通道配置的網路拓撲圖和 IP 位址工作表。
sidebar:
  order: 3
i18n:
...
**xDC_NAMEx** 資料中心連接至 Cloud 清洗中心的配置。

:::note
**以下為範例值。** 請使用上方表格中客戶專屬及 SOC 提供的值進行替換。

受保護的前綴**必須是可公開路由的**（非 RFC 1918）。
當通道經由公共網際網路傳輸時，GRE 外部端點 IP 也必須是可公開路由的；私有連線（L2、私有對等互連）可允許使用 RFC 1918 端點。請參閱
[K000147949](https://my.f5.com/manage/s/article/K000147949) 以取得使用正確文件位址的範例。

為實現冗餘，請為**每台 BIG-IP 設備建立 2 條通道**，分別連接至不同地理位置的清洗中心（HA 配對共 4 條通道）。
:::
...
在建立通道配置時，請參考以下 XC 和 BIG-IP 工作表。
...
:::note[內部（傳輸）IP]
內部 IP（如 `10.10.10.0/30`）使用 RFC 1918 位址。這是正確的，因為它們被封裝在 GRE 通道內部，不會出現在公共網際網路上。受保護的前綴必須始終是可公開路由的；當通道經由公共網際網路傳輸時，外部端點 IP 必須是可公開路由的。
:::

:::note[IPv6 內部連結]
此處 IPv6 內部連結使用 /64 前綴以匹配常見的 Cloud 預設值。對於點對點連結，根據 [RFC 6164](https://datatracker.ietf.org/doc/html/rfc6164) 建議使用 /127 以避免鄰居探索耗盡問題。如果 SOC 通道分配支援，請使用 /127。
:::

---
title: 驗證
description: 在 BIG-IP 上驗證 GRE 通道和 BGP 配置，並排除常見問題。
sidebar:
  order: 6
i18n:
...
1. 確認內部 self IP 的 `allow-service` 包含 `tcp:179`。
1. 透過通道 ping 遠端內部 IP。
1. 檢查兩端的 BGP 密碼是否一致（這是造成 `NOTIFICATION` 錯誤的常見原因）。
1. 確認 prefix-lists 和 route-maps 已正確套用。
1. 檢查 `show ip bgp neighbors <IP>` 以查看最後的錯誤/重設原因。
1. 在每個 HA 單元上，確認該單元自身的通道已啟動，且其自身的 BGP 鄰居已配置（而非另一個單元的）。
:::
